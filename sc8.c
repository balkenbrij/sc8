...
{
  SC8Image         sc8;
  BYTE             sc8header[7] = { 0xfe, 0x00, 0x00, 0x00, 0xd4, 0x00, 0x00 };
  ...
  switch (dopt) {
    case 1: strcpy(dither_type, "o2x2,8,8,4"); break;
    case 2: strcpy(dither_type, "o4x4,8,8,4"); break;
    case 3: strcpy(dither_type, "o8x8,8,8,4"); break;
  }
  ...
  if ((fout = fopen(output_name, "wb")) == NULL)
    exit_error("error: can't open output file\n");
  if (fwrite(&sc8header, sizeof(sc8header), 1, fout) != 1 ||
